...
public class VatrenaLoptica extends Loptica
{
	...
	public void unistiPolje(int i, int j, Blok[][] blokovi)
	{
		...
		// GORE
		if (i > 0 && blokovi[i - 1][j] != null && !blokovi[i - 1][j].isUnisten()) blokovi[i - 1][j].unistiSe(blokovi);

		// DOLE
		if (i < 19 && blokovi[i + 1][j] != null && !blokovi[i + 1][j].isUnisten()) blokovi[i + 1][j].unistiSe(blokovi);

		// LEVO
		if (j > 0 && blokovi[i][j - 1] != null && !blokovi[i][j - 1].isUnisten()) blokovi[i][j - 1].unistiSe(blokovi);

		// DESNO
		if (j < 19 && blokovi[i][j + 1] != null && !blokovi[i][j + 1].isUnisten()) blokovi[i][j + 1].unistiSe(blokovi);
